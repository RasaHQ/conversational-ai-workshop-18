...
class SimulatedUser(object):
    def __init__(self):
        ...
        self.preferences = {
            ...
            "people": ["1", "2", "4","6"],
            "cuisine": ["chinese", "italian", "french"],
    ...
    def response_correct(self, action):
        ...
        if intent == UserState.CHITCHAT.value:
            ...
            try:
                entity = action.split("utter_correct_")[1].split("_restaurant")[0]
                return action == "utter_correct_{}_restaurant".format(entity)
            except:
                return False            
        elif intent == UserState.DIDTHATWORK.value:
            if len(self.provided) < len(self.preferences):
                return action == "utter_more_info_restaurant"
            else:
                return action == "utter_worked_restaurant"
        elif intent == UserState.EXPLAIN.value:
            try:
                entity = user.last_system_utter.split("utter_ask_")[1]
                return action == "utter_explain_{}_restaurant".format(entity)
            except:
...
def run_sim_user(agent, max_steps=8):
    success = True
    user = SimulatedUser()
    first_utter = "request_restaurant"
